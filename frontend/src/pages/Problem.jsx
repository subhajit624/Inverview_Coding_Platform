...
import { Code2, ArrowLeft, Play, CheckCircle2, XCircle, ChevronRight, BookOpen, Terminal } from "lucide-react";
...
export default function Problem() {
  ...
  const onMouseMove = useCallback((e) => {
    ...
    setLeftWidth(Math.min(Math.max(pct, 20), 70)); // clamp 20%–70%
  }, []);
  ...
  const ProblemPanel = (
    <div className="h-full overflow-y-auto p-5 space-y-5">
      <div>
        <h1 className="text-xl font-black tracking-tight mb-2">{problem.title}</h1>
  ...
  return (
    <div className="h-screen flex flex-col text-white overflow-hidden"
      ...
      {/* MAIN — desktop: side-by-side with splitter | mobile: tabs */}
      {isMobile ? (
        // ── MOBILE ──────────────────────────────────────────────────────────
